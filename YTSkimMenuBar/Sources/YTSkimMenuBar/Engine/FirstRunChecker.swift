...
final class FirstRunChecker: @unchecked Sendable {
  ...
  func runChecks() async -> [CheckItem] {
    ...
    if let scriptURL = runner.bundledScriptURL(), FileManager.default.isReadableFile(atPath: scriptURL.path) {
      items.append(CheckItem(
        title: "Bundled script",
        status: .pass,
        details: "Found bundled yt-skim.sh at \(scriptURL.path).",
      ...
      items.append(CheckItem(
        title: "Bundled script",
        status: .fail,
        details: "Bundled yt-skim.sh was not found in app resources.",
        remediation: "Rebuild the app and confirm Resources/yt-skim.sh is included."
      ))
    }
