...
final class ScriptRunner: @unchecked Sendable {
  ...
  func bundledScriptURL() -> URL? {
    Bundle.module.url(forResource: scriptName, withExtension: "sh", subdirectory: "Resources")
  }
  ...
  private func runScript(args: [String]) async throws -> (stdout: String, stderr: String, exitCode: Int32) {
    ...
    let defaults = ["/opt/homebrew/bin", "/usr/local/bin", "/usr/bin", "/bin", "/Applications/Codex.app/Contents/Resources"]
    let mergedPath = (defaults + [existingPath]).joined(separator: ":")
    env["PATH"] = mergedPath
