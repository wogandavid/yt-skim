...
final class AppModel: ObservableObject {
  ...
  func summarizeClipboard() {
    ...
    Task {
      ...
      let trimmed = sanitizeClipboard(clipboard)
      guard let url = URL(string: trimmed), isYouTubeURL(url) else {
        setFailure(
          host: "Clipboard",
          message: "Couldn't summarize. Not a YouTube link.",
          details: "Clipboard content is not a supported YouTube URL: \(trimmed)"
        )
        return
      }
